...
public class MantenimientoController implements Serializable {
    ...
    protected void setEmbeddableKeys() {
        selected.getMantenimientoPK().setClientecedula(selected.getCliente().getCedula());
        selected.getMantenimientoPK().setVeihculoplaca(selected.getVeihculo().getPlaca());
    }

    protected void initializeEmbeddableKey() {
        selected.setMantenimientoPK(new modulo.MantenimientoPK());
    }
    ...
    public Mantenimiento getMantenimiento(modulo.MantenimientoPK id) {
    ...
    public static class MantenimientoControllerConverter implements Converter {
        ...
        modulo.MantenimientoPK getKey(String value) {
            modulo.MantenimientoPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new modulo.MantenimientoPK();
            key.setClientecedula(Integer.parseInt(values[0]));
            key.setVeihculoplaca(values[1]);
            return key;
        }

        String getStringKey(modulo.MantenimientoPK value) {
            StringBuilder sb = new StringBuilder();
            sb.append(value.getClientecedula());
            sb.append(SEPARATOR);
            sb.append(value.getVeihculoplaca());
            return sb.toString();
        }
